 ...
 * Versi: 134.0 (BASE 130.0 - CLEAN DROPDOWN - LOCKED LOADING ENGINE)
 */
...
function getDashboardData() {
  ...
  // FUNGSI PENYARING CABANG (Mencegah produk masuk ke list)
  var isPureBranch = function(val) {
    if (!val) return false;
    var v = val.toString().toUpperCase();
    // Jika mengandung kata-kata di bawah, maka BUKAN Cabang
  ...
  try {
    // 1. DATA ACHIEVEMENT (DASHBOARD HOME)
    ["Raw_Achv_CS", "Raw_Achv_SPV", "Raw_Achv_Tele"].forEach(name => {
      ...
      data.slice(1).forEach(r => {
        ...
        // Populasi list cabang dashboard dengan penyaring ketat
        if (isPureBranch(cab) && res.listCHome.indexOf(cab) === -1) {
          res.listCHome.push(cab);
        }
        res.achv.push([tgl, cab, String(r[2]).trim(), key, Number(r[3])||0, Number(r[4])||0]);
      });
    });

    // 2. DATA PENCAIRAN (DASHBOARD HOME)
    ...
    // 3. DATA PIPELINE (PIPELINE MODULE)
    ...
    if (shP && shP.getLastRow() >= 1) {
      ...
      dataP.forEach(r => {
        ...
        // Populasi list cabang pipeline dengan penyaring ketat
        if (isPureBranch(cabP) && res.listCPipe.indexOf(cabP) === -1) {
          res.listCPipe.push(cabP);
        }
        res.pipeline.push({ tgl: tglP, deb: debP, cab: cabP, st: String(r[4]).trim(), kep: String(r[6]).trim(), sal: salP, spv: spvP, pla: plaP });
