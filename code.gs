 ...
 * Versi: 127.0 (TOTAL SYNC - MATCHING HEADER & FOOTER - FINAL)
 */
...
function getDashboardData() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var res = { u: getUserData(), achv: [], cair: [], pipeline: [], listC: [], listS: [], listSPV: [] };

  try {
    // 1. DATA ACHIEVEMENT (CS, SPV, TELE)
    ["Raw_Achv_CS", "Raw_Achv_SPV", "Raw_Achv_Tele"].forEach(name => {
      ...
      data.slice(1).forEach(r => {
        ...
        if (res.listC.indexOf(cab) === -1) res.listC.push(cab);
        res.achv.push([tgl, cab, String(r[2]).trim(), key, Number(r[3])||0, Number(r[4])||0]);
      });
    });

    // 2. DATA PENCAIRAN (DETAIL PRODUK)
    ...
    // 3. DATA PIPELINE (KOLOM B & J)
    var shP = ss.getSheetByName('Raw_Pipeline');
    if (shP && shP.getLastRow() >= 1) {
      var dataP = shP.getDataRange().getValues();
      dataP.forEach(r => {
        if (!r[1] || String(r[1]).toLowerCase().includes("debitur") || !r[0]) return;
        var tglP = (r[0] instanceof Date) ? Utilities.formatDate(r[0], "GMT+7", "yyyy-MM-dd") : String(r[0]).substring(0,10);
        res.pipeline.push({
          tgl: tglP, deb: String(r[1]), cab: String(r[2]), st: String(r[3]), 
          kep: String(r[5]), sal: String(r[6]), spv: String(r[7]), pla: Number(String(r[9]||"0").replace(/[^0-9.-]+/g,"")) || 0
        });
        if (r[6] && res.listS.indexOf(String(r[6])) === -1) res.listS.push(String(r[6]));
        if (r[7] && res.listSPV.indexOf(String(r[7])) === -1) res.listSPV.push(String(r[7]));
      });
    }
  } catch(e) { }
  return res;
